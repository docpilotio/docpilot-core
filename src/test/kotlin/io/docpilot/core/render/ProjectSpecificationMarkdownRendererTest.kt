...
class ProjectSpecificationMarkdownRendererTest {
    ...
    fun `renders all DIR sections into deterministic markdown artifacts`() {
        ...
        assertTrue(artifacts.any { it.relativePath == "docs/architecture/module-architecture.md" })
        assertTrue(artifacts.any { it.relativePath == "docs/contracts/domain-model.md" })
        assertTrue(artifacts.any { it.relativePath == "docs/contracts/database-schema.md" })
        assertTrue(artifacts.any { it.relativePath == "docs/contracts/external-apis.md" })
        assertTrue(artifacts.any { it.relativePath == "docs/quality/test-strategy.md" })
        assertTrue(content.contains("# Module Architecture"))
        assertTrue(content.contains("# Domain Model"))
        assertTrue(content.contains("# Database Schema"))
        assertTrue(content.contains("# External API Contract"))
        assertTrue(content.contains("# Test Strategy"))
        assertTrue(content.contains("legitimately empty deterministic result"))
