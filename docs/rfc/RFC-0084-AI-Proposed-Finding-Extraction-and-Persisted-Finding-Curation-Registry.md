...
Status: `PARTIALLY_IMPLEMENTED` — Part A (`generate propose-findings`) is implemented and verified; Part B (persisted Finding/curation registry) has not been started. The user explicitly chose to implement Part A first and defer Part B.
...
5. `generate findings` merges newly-validated Findings into the persisted registry without duplicating an already-present `Finding.id`.
6. `generate roadmap --decisions-registry` and `generate adr-adopt --decisions-registry` both read-merge-apply-persist against the same decisions store, and a decision made via one command is visible to a later invocation of either.

Both parts:

7. No file under `src/main/kotlin/io/docpilot/core/documentation/{adr,advisory,backlog,synthesis}/` or `src/main/kotlin/io/docpilot/core/specification/finding/{Finding,FindingFactory}.kt` is modified. **Met for Part A.**
8. Full `./gradlew test` (all modules) passes with 0 failures. **Met for Part A** — 549 tests, 0 failures.
